...
# Преобразование размерности для RandomForestClassifier
X_train_flat = X_train.reshape(X_train.shape[0], -1)
X_test_flat = X_test.reshape(X_test.shape[0], -1)
X_val_flat = X_val.reshape(X_val.shape[0], -1)

# Метод Случайные леса (Random Forests)
...
sample_image = X_val_flat[0]
start_time_single = time.time()
prediction_single = rf_model.predict([sample_image])
